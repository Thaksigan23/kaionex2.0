...
export function ProductSnippet({ product, compact = false }: { product: ProductId; compact?: boolean }) {
  return (
    <span className={"kx-snippet " + (compact ? "kx-snippet-compact" : "")}>
      ...
      {product === "ecommerce" && <><span className="kx-snippet-row"><span>Order #1048</span><span className="kx-signal">Ready</span></span><span className="kx-snippet-row"><span>Stock on hand</span><b>26 units</b></span><span className="kx-meter"><i style={{ width: "82%" }} /></span></>}
...
export function OrderManifest() {
  return <div className="kx-manifest"><span className="kx-workforce-icon"><Package size={20} /></span><div><strong>Fulfillment queue</strong><span>Sample order #1048 · 1 item</span></div><span className="kx-signal">Ready for packing</span></div>;
}
export function ConnectedWorkflow() {
  return <div className="kx-workflow" aria-label="Illustrative KAIONEX product family">
    <div><span className="kx-window-label">01 · Counter</span><strong><Coffee size={16} /> KAIONEX POS</strong><ProductSnippet product="pos" compact /></div>
    <ArrowRight className="kx-flow-arrow" aria-hidden />
    <div className="kx-core-tile"><span className="kx-window-label">02 · Product Suite</span><strong>KAIONEX</strong><span>Dedicated Software</span><span className="kx-signal">Business Portfolio</span></div>
    <ArrowRight className="kx-flow-arrow" aria-hidden />
    <div><span className="kx-window-label">03 · Operations</span><strong>FMS & E-Commerce</strong><ProductSnippet product="fms" compact /><span className="kx-signal">Purpose-built tools</span></div>
  </div>;
}
export function WorkspaceActivity({ view = "overview" }: { view?: string }) {
  const labels: Record<string, string[]> = {
    overview: ["POS · Sale completed", "EMS · Restock task completed", "E-Commerce · Order received"],
    sales: ["POS · Checkout completed", "POS · Receipt printed", "POS · Cash drawer balanced"],
    inventory: ["POS · Counter stock recorded", "E-Commerce · Catalog updated", "EMS · Shelf prep assigned"],
    finance: ["FMS · Daily entry recorded", "FMS · Invoice issued", "FMS · Ledger balanced"],
    employees: ["EMS · Morning shift active", "EMS · Task assigned", "EMS · Shift check-in complete"],
    analytics: ["POS · Sales summary", "E-Commerce · Storefront analytics", "FMS · Financial statement"],
  };
  return <div className="kx-workspace-detail">
    ...
    <div className="kx-activity-panel"><span className="kx-window-label">Product activity · Demo</span>{(labels[view] ?? labels.overview).map((label, i) => <div className="kx-activity-row" key={label}><span className="kx-activity-dot" /><div><strong>{label}</strong><span>Illustrative event · Step {i + 1}</span></div><Check size={13} /></div>)}</div>
  </div>;
}
