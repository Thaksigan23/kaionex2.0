...
const groups = [
  { title: "One Connected Platform", icon: Layers3, items: [0], signals: ["POS", "EMS", "FMS", "E-Commerce"] },
...
export function WhyKaionex() {
  return <section className="cine-benefits">
    <div className="cine-benefits-header"><p className="cine-eyebrow">WHY KAIONEX</p><h2 className="cine-heading">Less fragmentation.<br /><em>More perspective.</em></h2><p>Connected products are only the beginning.<br />What matters is how your business works together.</p></div>
    <div className="cine-benefits-list">{groups.map(({ title, icon: Icon, items, signals }, index) => <Reveal key={title} y={12}>
      <article className={"cine-benefit " + (index === 0 ? "cine-benefit-anchor" : "")}>
        ...
        <div className="cine-benefit-content">{items.map(i => <p key={i}>{whyKaionex[i].description}</p>)}<div className="cine-benefit-signals" aria-hidden>{signals.map((label,i) => <span key={label}>{label}{i < signals.length - 1 && <ArrowRight size={12} />}</span>)}</div>{index === 0 && <span className="cine-benefit-future">KAIONEX CORE · CRM Coming Soon</span>}</div>
      </article>
    </Reveal>)}</div>
  </section>;
}
